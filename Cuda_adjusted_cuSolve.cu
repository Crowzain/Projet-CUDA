...
void computeEigenvalues(cusolverDnHandle_t cusolverH, int matrix_size,
    double* d_A, double* d_W, cudaStream_t stream) {
// Nombre d’éléments pour le workspace
int lwork = 0;
double *d_work = NULL;
int *devInfo = NULL;
cudaMalloc((void**)&devInfo, sizeof(int));

// On choisit l’algorithme : ici, on utilise la fonction d’diagonalization pour
// matrice symétrique (double precision) en utilisant potrf ou syevd (ici syevd)
cusolverEigMode_t jobz = CUSOLVER_EIG_MODE_NOVECTOR; // Calcul seulement des valeurs propres
cublasFillMode_t uplo = CUBLAS_FILL_MODE_UPPER; // ou LOWER selon comment vous stockez la matrice

// Première étape : requête de la taille du workspace
cusolverDnDsyevd_bufferSize(
cusolverH, jobz, uplo, matrix_size, d_A, matrix_size, d_W, &lwork);

cudaMalloc((void**)&d_work, lwork * sizeof(double));

// On définit le stream sur le handle cuSOLVER
cusolverDnSetStream(cusolverH, stream);

// Appel de la fonction qui calcule les valeurs propres
cusolverDnDsyevd(
cusolverH, jobz, uplo, matrix_size, d_A, matrix_size, d_W,
d_work, lwork, devInfo);

// On peut ajouter ici la vérification de devInfo pour s’assurer de la convergence
int info_gpu = 0;
cudaMemcpy(&info_gpu, devInfo, sizeof(int), cudaMemcpyDeviceToHost);
if (info_gpu != 0) {
fprintf(stderr, "Echec : info = %d\n", info_gpu);
}
...
{
    FILE *f_eigen;
    FILE *metrics64 = fopen("metrics_cuSolve_64threadsPerBlock.csv", "a");
    ...
    fprintf(metrics64, "%d;", numMatrices);
    /* --------------------- Host Computation --------------------- */
    /* printf("\n------ Host ------\n");
    double *h_eigs_cpu;
    int N = n * n;
...
float occupancyVector[NUM_STREAMS] = {0.0f};
double start = getTimeInMs();
...
for (int s = 0; s < NUM_STREAMS; s++)
{
    ...
    double *d_A_chunk = NULL, *d_eigs_chunk = NULL;
    cudaMalloc((void **)&d_A_chunk, chunkMatrixBytes);
    cudaMalloc((void **)&d_eigs_chunk, chunkEigBytes);

    // Asynchrone copy of the chunk matrices to the device
    cudaMemcpyAsync(d_A_chunk, h_A_gpu + startMat * n * n,
                               chunkMatrixBytes, cudaMemcpyHostToDevice, streams[s]);

    // For each matrix in the chunk, compute the eigenvalues
    
    for (int m = 0; m < numChunk; m++)
    {
        double *d_A_mat = d_A_chunk + m * n * n;
        double *d_eigs_mat = d_eigs_chunk + m * n;
        computeEigenvalues(cusolverH, n, d_A_mat, d_eigs_mat, streams[s]);
    }

    // Asynchrone copy of the chunk eigenvalues to the host
...
printf("Total GPU processing time for %d matrices (n=%d): %f ms\n", numMatrices, n, t_gpu);
    ...
    double ops = (2 * n * n * n * n * numMatrices);
    double perf = ops / (t_gpu * 1000.0 * 1000.0);
    double throughput = totalMatrixSize / t_gpu / 1000.0 / 1000.0;
    double avgLatency = t_gpu * 1000.0 * 1000.0 / totalMatrixSize;

    fprintf(metrics64, "%f;", t_gpu);
    fprintf(metrics64, "%f;", throughput);
    fprintf(metrics64, "%f;", perf);
...
if (argc >= 3)
{
    ...
    if (f_eigen)
    {
        ...
        fclose(f_eigen);
        printf("Total error compared to reference (GPU): %.14f\n", error);
        fprintf(metrics64, "%.14f\n", error);
    }
    else
    {
        fprintf(stderr, "Reference eigenvalue file not found.\n");
    }
}
/* printf("\n------ Summary ------\n");
